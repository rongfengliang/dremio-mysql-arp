...
public final class MyMySQLDialect extends ArpDialect {
    private final ArpTypeMapper typeMapper;

    public MyMySQLDialect(ArpYaml yaml) {
        super(yaml);
        this.typeMapper = new MyMySQLDialect.MyMySQLTypeMapper(yaml);
    }
    ...
    public MyMySQLDialect.MyMySQLSchemaFetcher newSchemaFetcher(JdbcPluginConfig config) {
        ...
        return new MyMySQLDialect.MyMySQLSchemaFetcher(query, config);
    }
    ...
    private static class MyMySQLTypeMapper extends ArpTypeMapper {
        public MyMySQLTypeMapper(ArpYaml yaml) {
            super(yaml);
        }
    ...
    private static final class MyMySQLSchemaFetcher extends ArpDialect.ArpSchemaFetcher {
        private static final Logger logger = LoggerFactory.getLogger(MyMySQLDialect.MyMySQLSchemaFetcher.class);

        private MyMySQLSchemaFetcher(String query, JdbcPluginConfig config) {
            super(query, config);
        }
